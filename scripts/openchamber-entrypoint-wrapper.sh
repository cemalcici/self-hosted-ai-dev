...
# to the aidev user before launching the server.
#
# Docker named volumes attach as root:root.  The aidev server
...
# Also ensure the workspace volume is writable by the aidev user
chown -R 1000:1000 /workspace 2>/dev/null || true

# Verify the aidev user exists
...
exec runuser -u "$USER" -- sh -c "
  ...
  # 'already running' when a previous container crashed and left an orphaned PID.
